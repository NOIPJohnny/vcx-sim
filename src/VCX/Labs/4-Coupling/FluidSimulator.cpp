...
namespace VCX::Labs::Coupling {
    ...
    namespace {
        ...
        inline void rebuildCellTypesImpl(std::vector<int> & type,
    ...
    int Simulator::index2GridOffset(glm::ivec3 index) const {
    ...
    void Simulator::rebuildCellTypes() {
        rebuildCellTypesImpl(
            m_type, m_s, m_particlePos, glm::vec3(-0.5f), m_fInvSpacing,
            m_iCellX, m_iCellY, m_iCellZ,
            [&](glm::ivec3 index) { return index2GridOffset(index); });
    }

    void Simulator::resetSolidMask() {
